(() => {
  ...
  const init = () => {
    ...
    document.querySelectorAll('.app-notifications').forEach((widget) => {
      ...
      const setCleared = () => { badge.hidden = true; list.innerHTML = '<div class="app-notifications__empty">Henüz yeni bir bildirimin yok.</div>'; };
      if (localStorage.getItem('hesapmatik-notifications-cleared') === 'true') setCleared();
      toggle.addEventListener('click', (event) => { event.stopPropagation(); const open = widget.classList.toggle('is-open'); toggle.setAttribute('aria-expanded', open); });
      clear.addEventListener('click', () => { localStorage.setItem('hesapmatik-notifications-cleared', 'true'); setCleared(); });
      document.addEventListener('click', (event) => { if (!widget.contains(event.target)) { widget.classList.remove('is-open'); toggle.setAttribute('aria-expanded', 'false'); } });
    });
  };
